...
# Importa il modello InceptionV3 dalle applicazioni TensorFlow Keras
import tensorflow.keras.applications.inception_v3

# Carica il modello InceptionV3 con le impostazioni specificate
...
# Congela i livelli per evitare che vengano addestrati sul nuovo dataset
for layer in inception_v3.layers:
    layer.trainable = False
# Visualizza un riepilogo del modello InceptionV3
inception_v3.summary()
# Ottieni il livello 'mixed7' come livello di output per il tuo modello personalizzato
last_layer = inception_v3.get_layer('mixed7')
last_output = last_layer.output

# Definisci ulteriori livelli per il tuo modello personalizzato
x = tf.keras.layers.Flatten()(last_output)
x = tf.keras.layers.Dense(100, activation='relu')(x)
x = tf.keras.layers.Dropout(0.2)(x)
x = tf.keras.layers.Dense(2, activation='softmax')(x)
# Crea il tuo modello pre-addestrato con i livelli personalizzati
pre_trained_model = tf.keras.models.Model(inputs=inception_v3.input, outputs=x)

# Visualizza un riepilogo del tuo modello pre-addestrato
pre_trained_model.summary()
# Compila il tuo modello con perdita, ottimizzatore e metriche specificate
pre_trained_model.compile(loss=tf.keras.losses.categorical_crossentropy, optimizer=tf.keras.optimizers.Adam(),
                          metrics=['accuracy'])
# Addestra il modello sui dati di addestramento per 50 epoche con dati di convalida
history_pre_trained = pre_trained_model.fit(train_d_gen, epochs=50, validation_data=val_d_gen)
# Estrai perdita di addestramento, perdita di convalida, accuratezza di addestramento e accuratezza di convalida
...
# Traccia la perdita di addestramento e di convalida
plt.figure(figsize=(12, 6))
plt.plot(loss, 'b', label='training loss')
plt.plot(val_loss, 'orange', label='validation loss')
plt.legend()
plt.show()
# Traccia l'accuratezza di addestramento e di convalida
plt.figure(figsize=(12, 6))
plt.plot(acc, 'b', label='training accuracy')
plt.plot(val_acc, 'orange', label='validation accuracy')
plt.legend()
plt.show()
# Valuta il modello sui dati di convalida
pre_trained_model.evaluate(val_d_gen)
# Stampa i risultati della valutazione
print(pre_trained_model.evaluate(val_d_gen))
